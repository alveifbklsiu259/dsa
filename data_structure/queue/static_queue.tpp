...
QUEUE_TEMPLATE T* StaticQueue<T, N>::getBuffer(size_t i) noexcept {
  return reinterpret_cast<T*>(m_data.data()) + i; // NOLINT
}

QUEUE_TEMPLATE const T* StaticQueue<T, N>::getBuffer(size_t i) const noexcept {
  return reinterpret_cast<const T*>(m_data.data()) + i; // NOLINT
}

QUEUE_TEMPLATE template <size_t M>
void StaticQueue<T, N>::move(
    StaticQueue<T, M>&& other
...
QUEUE_TEMPLATE template <size_t M> void StaticQueue<T, N>::copy(const StaticQueue<T, M>& other) {
  static_assert(M <= N, "Source queue too large for target queue");
  m_length = other.m_length;
  m_head = 0;
  m_tail = m_length;
...
QUEUE_TEMPLATE StaticQueue<T, N>::StaticQueue(const StaticQueue<T, N>& other) { copy(other); }
QUEUE_TEMPLATE template <size_t M> StaticQueue<T, N>::StaticQueue(const StaticQueue<T, M>& other) {
  copy(other);
}

QUEUE_TEMPLATE StaticQueue<T, N>& StaticQueue<T, N>::operator=(const StaticQueue<T, N>& other) {
...
QUEUE_TEMPLATE template <size_t M>
StaticQueue<T, N>& StaticQueue<T, N>::operator=(const StaticQueue<T, M>& other) {
  copy(other);
  return *this;
}

QUEUE_TEMPLATE
StaticQueue<T, N>::StaticQueue(StaticQueue<T, N>&& other) noexcept(std::is_nothrow_move_constructible_v<T>) {
  move(std::move(other));
}

QUEUE_TEMPLATE template <size_t M>
StaticQueue<T, N>::StaticQueue(StaticQueue<T, M>&& other) noexcept(std::is_nothrow_move_constructible_v<T>) {
  move(std::move(other));
}

QUEUE_TEMPLATE StaticQueue<T, N>&
