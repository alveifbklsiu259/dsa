...
QUEUE_TEMPLATE template <size_t M> void Queue<T, N>::move(Queue<T, M> &&other) {
...
void Queue<T, N>::copy(const Queue<T, M> &other) {
...
QUEUE_TEMPLATE template <size_t M>
Queue<T, N>::Queue(const Queue<T, M> &other) {
  copy(other);
}
...
QUEUE_TEMPLATE template <size_t M>
Queue<T, N> &Queue<T, N>::operator=(const Queue<T, M> &other) {
  init();
  copy(other);
  return *this;
}

QUEUE_TEMPLATE Queue<T, N>::Queue(Queue<T, N> &&other) noexcept {
  move(std::move(other));
}

QUEUE_TEMPLATE template <size_t M>
Queue<T, N>::Queue(Queue<T, M> &&other) noexcept {
  move(std::move(other));
}
...
QUEUE_TEMPLATE size_t Queue<T, N>::getCapacity() const noexcept {
  return m_capacity;
}
QUEUE_TEMPLATE bool Queue<T, N>::isFull() const noexcept {
  return m_length == m_capacity;
}
QUEUE_TEMPLATE bool Queue<T, N>::isEmpty() const noexcept {
  return m_length == 0;
}
